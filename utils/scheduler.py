...
def get_scheduler(config):
    ...
    if config.type == 'STEP':
        return StepLRScheduler(config.optimizer, config.lr_steps, config.lr_mults, config.base_lr, config.warmup_lr,
                               config.warmup_steps, last_iter=config.last_iter)
    elif config.type == 'COSINE':
        return CosineLRScheduler(config.optimizer, config.max_iter, config.min_lr, config.base_lr, config.warmup_lr,
                                 config.warmup_steps, last_iter=config.last_iter)
    else:
